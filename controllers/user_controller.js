...
// MW que permite acciones solamente si el usuario creador del objeto
// corresponde con el usuario logueado o si es cuenta admin
	...
	models.User.find({where: { id: Number(userId)}})
		.then(function(user) {
			if (user) {
				req.user = user;
				next();
			} else {next(new Error('No existe usuario: ' + userId))}
		}
...
exports.edit = function(req, res) {
	res.render('user/edit', { user: req.user, errors: []}); // req.user usuario cargado con autoload
};
